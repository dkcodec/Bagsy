...
const extractNationalFromDisplay = (
  display: string,
  callingCode: string
) => {
...
const formatDisplay = (callingCode: string, nationalDigits: string, countryCode: Country["code"]) => {
...
export const PhoneInput = React.forwardRef<HTMLInputElement, PhoneInputProps>(
  (
    ...
    React.useEffect(() => {
      setDisplay(formatDisplay(country.callingCode, nationalFromValue, country.code))
    ...
    const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      setDisplay(formatDisplay(country.callingCode, nextNational, country.code))
      onChange?.(nextE164)
    }
    ...
    const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
      ...
      const currentNational = extractNationalFromDisplay(display, country.callingCode)
                ...
                {countries.map((c) => (
                  <li key={c.code}>
                    ...
                    >
                      ...
                      <span className="text-foreground/60">+{c.callingCode}</span>
                    </button>
                  </li>
                ))}
              </ul>
